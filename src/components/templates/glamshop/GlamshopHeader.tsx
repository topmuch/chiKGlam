...
export default function GlamshopHeader() {
  ...
  const handleOpenLogin = () => { setLoginDefaultMode('login'); setLoginOpen(true); };
  const handleOpenRegister = () => { setLoginDefaultMode('register'); setLoginOpen(true); };
  ...
  const handleNavMouseEnter = useCallback((item: { label: string; action: string; slug?: string }) => {
    ...
    if (item.action === 'home' || item.action === 'contact-page' || item.action === 'espace-client' || item.action === 'boutique') {
      setActiveCategory(null); setMegaMenuOpen(false); return;
    }
  ...
  const handleNavMouseLeave = useCallback(() => {
    navTimeoutRef.current = setTimeout(() => { setMegaMenuOpen(false); setActiveCategory(null); }, 200);
  }, []);

  const handleNavClick = useCallback((item: { label: string; action: string; slug?: string }) => {
    if (item.action === 'home') navigateTo('home');
    else if (item.action === 'contact-page') navigateTo('contact-page');
    else if (item.action === 'espace-client') handleOpenLogin();
    else if (item.action === 'category' && item.slug) navigateTo('category', { category: item.slug });
  }, [navigateTo]);
  ...
  return (
    <>
      ...
      {promoBarVisible && (
        <div className="text-white relative z-50" style={{ backgroundColor: '#bc8752' }}>
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-center h-10">
            ...
            <button onClick={() => setPromoBarVisible(false)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/70 hover:text-white transition-colors" aria-label="Fermer">
              <X className="size-4" />
            </button>
          </div>
        </div>
      ...
      <header className={`sticky top-0 z-50 w-full bg-white transition-shadow duration-300 ${isScrolled ? 'shadow-sm' : ''}`}>
              ...
              {currentUser ? (
                <div className="relative group">
                  <Button variant="ghost" size="icon" className="shrink-0 relative" aria-label="Mon Compte">
                    <div className="size-7 rounded-full text-white flex items-center justify-center text-xs font-semibold" style={{ backgroundColor: '#bc8752' }}>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl border border-border shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div className="px-4 py-3 border-b border-border">
                      <div className="flex items-center gap-2">
                        <div className="size-9 rounded-full text-white flex items-center justify-center text-sm font-semibold shrink-0" style={{ backgroundColor: '#bc8752' }}>
                          {(currentUser.name || currentUser.email).charAt(0).toUpperCase()}
                        </div>
                    ...
                    <div className="py-1">
                      <button onClick={() => navigateTo('customer-dashboard')} className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-left hover:bg-secondary transition-colors">
                        <User className="size-4 text-muted-foreground" /> Mon Compte
                      </button>
                      {currentUser.role === 'admin' && (
                        <>
                          <button onClick={() => navigateTo('admin-dashboard')} className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-left hover:bg-secondary transition-colors" style={{ color: '#bc8752' }}>
                            <LayoutDashboard className="size-4" /> Tableau de Bord
                          </button>
                          <Separator />
                          <button onClick={() => navigateTo('category', { category: 'boutique' })} className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-left hover:bg-secondary transition-colors">
                            <Package className="size-4 text-muted-foreground" /> Voir la Boutique
                          </button>
                        </>
                      )}
                      <Separator />
                      <button onClick={handleSignOut} className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-left text-destructive hover:bg-secondary transition-colors">
                        <X className="size-4" /> Se Déconnecter
                      </button>
                    </div>
                  </div>
                </div>
              ) : (
                <Button variant="ghost" size="icon" className="shrink-0" aria-label="Se connecter" onClick={handleOpenLogin}>
                  <LogIn className="size-5" style={{ color: '#bc8752' }} />
                </Button>
              )}
              <Button variant="ghost" size="icon" className="hidden sm:inline-flex shrink-0" aria-label="Wishlist">
                <Heart className="size-5" style={{ color: '#bc8752' }} />
              </Button>
              <Button variant="ghost" size="icon" className="relative shrink-0" onClick={() => setCartOpen(true)} aria-label="Shopping bag">
                <ShoppingBag className="size-5" style={{ color: '#bc8752' }} />
                {itemCount > 0 && (
                  <Badge className="absolute -top-1 -right-1 size-5 flex items-center justify-center p-0 text-[10px] border-0 rounded-full text-white" style={{ backgroundColor: '#bc8752' }}>
                  ...
                  <button
                    ...
                    className={`px-5 py-3 text-sm font-medium tracking-wide uppercase transition-colors whitespace-nowrap ${
                      activeCategory === (item.slug || item.label)
                        ? 'border-b-2'
                        : 'text-gray-600 hover:text-gray-900 hover:border-b-2'
                    }`}
                    style={{ borderColor: activeCategory === (item.slug || item.label) ? '#bc8752' : 'transparent', color: activeCategory === (item.slug || item.label) ? '#bc8752' : undefined }}
                  >
                    {item.label}
                  </button>
                </li>
        ...
        <div onMouseEnter={() => { if (navTimeoutRef.current) { clearTimeout(navTimeoutRef.current); navTimeoutRef.current = null; } }} onMouseLeave={handleNavMouseLeave}>
          <MegaMenu isOpen={megaMenuOpen} onClose={() => { setMegaMenuOpen(false); setActiveCategory(null); }} activeCategory={activeCategory} />
        </div>
      </header>
      <MobileMenu open={mobileMenuOpen} onClose={() => setMobileMenuOpen(false)} />
      <LoginDialog open={loginOpen} onOpenChange={setLoginOpen} defaultMode={loginDefaultMode} />
    </>
  );
}
