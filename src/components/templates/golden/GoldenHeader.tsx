...
const navItems = [
  ...
  { label: 'SEDUCTION BOX', action: 'category' as const, slug: 'box-de-seduction' },
...
export default function GoldenHeader() {
  ...
  const activeNav = useMemo(() => {
    ...
    if (currentPage === 'category') {
      ...
      if (selectedCategory === 'box-de-seduction') return 'SEDUCTION BOX';
      return 'BOUTIQUE';
    }
      ...
      {promoBarVisible && (
        ...
        >
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-center h-10">
            <p className="text-sm font-medium tracking-wide text-center" style={{ color: C.black }}>
              ...
              {navItems.map((item) => (
                <li key={item.label} className="relative">
                  <button
                    ...
                    className="relative px-4 py-3 text-[13px] font-medium tracking-[0.08em] uppercase transition-colors duration-200 whitespace-nowrap"
                    style={{
                      color: activeNav === item.label ? C.black : C.black,
                    }}
                    onMouseEnter={(e) => {
                      if (activeNav !== item.label) e.currentTarget.style.color = C.primaryDark;
                    }}
                    onMouseLeave={(e) => {
                      if (activeNav !== item.label) e.currentTarget.style.color = C.black;
                    }}
                  >
                    {item.label}
